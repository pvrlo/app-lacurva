// reserva_habitacion_screen.dart
...
class ReservaHabitacionScreen extends StatefulWidget {
  ...
  @override
  _ReservaHabitacionScreenState createState() =>
      _ReservaHabitacionScreenState();
}
...
class _ReservaHabitacionScreenState extends State<ReservaHabitacionScreen> {
  ...
  final String clientId =
      'AbPN2Ugwee2ogiTmjktpS49PC-fOdfwuZ-zMnEp8wRxVxCU_L7q_gHct-el1XBbR6M224kyh8RmQENz9'; // Reemplaza con tu Client ID de PayPal
  final String secret =
      'EOhuOzMc_C2_RxKTofr_6sbHkdEEKRaqSVV03m_vYb0fuNJkWc-JQ-tcBOP_pHvs6HPw4hw2r7qNImlP'; // Reemplaza con tu Secret de PayPal
  ...
  void initState() {
    ...
    print('ReservaHabitacionScreen - checkInDate: $checkInDate');
    print('ReservaHabitacionScreen - checkOutDate: $checkOutDate');
  }
  ...
  Future<void> _createPayPalPayment() async {
    ...
    if (accessToken == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('No se pudo obtener el token de acceso')));
      return;
    }
    ...
    final response = await http.post(
      ...
      body: json.encode({
        ...
        "redirect_urls": {
          "return_url":
              "https://example.com/success", // URL de retorno a tu servidor PHP
    ...
    if (response.statusCode == 201) {
      ...
      if (approvalUrl != null && await canLaunch(approvalUrl)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('No se pudo abrir PayPal')));
      }
    } else {
      print('Error al crear el pago: ${response.body}');
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error al crear el pago')));
    }
  }

  Future<void> _createReservation(String paymentId) async {
    final url = Uri.parse(
        'http://localhost/la_curva/crear_reserva.php'); // URL de tu archivo PHP
    ...
    String fechaEntrada = checkInDate != null
        ? DateFormat('yyyy-MM-dd').format(checkInDate!) // Formato YYYY-MM-DD
        : '';

    String fechaSalida = checkOutDate != null
        ? DateFormat('yyyy-MM-dd').format(checkOutDate!) // Formato YYYY-MM-DD
        : '';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text('Error al crear la reserva: ${response.statusCode}')));
        print('Error al crear la reserva: ${response.body}');
      }
    } catch (e) {
      print('Excepción al crear la reserva: $e');
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Excepción al crear la reserva')));
    }
  }
            ...
            child: Column(
              children: [
                ...
                TextFormField(
                  ...
                  onChanged: (value) {
                    setState(() {
                      adultos = int.tryParse(value) ??
                          1; // Predeterminado a 1 si el valor es inválido
                    });
                  },
                ),
                SizedBox(height: 16),
                TextFormField(
                  ...
                  onChanged: (value) {
                    setState(() {
                      ninos = int.tryParse(value) ??
                          0; // Predeterminado a 0 si el valor es inválido
                    });
                  },
                ),
                        ...
                        onPressed: () {
                          if (_formKey.currentState!.validate()) {
                            if (checkOutDate!.isBefore(checkInDate!)) {
                              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                  content: Text(
                                      'La fecha de check-out no puede ser anterior a la de check-in')));
                              return;
                            }
                            ...
                            _createPayPalPayment().then((_) {
                              ...
                              _createReservation('id_pago_simulado').then(
                                  (_) {
